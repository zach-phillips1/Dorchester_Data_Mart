...
While vial diltiazem remains available, it presents operational challenges:
- Requires refrigeration, or
- Has a 30-day expiration if stored unrefrigerated
To inform medication stocking decisions and reduce unnecessary logistical burden, Dorchester EMS reviewed historical medication utilization using its internal data mart.

---

## Data Source
Analysis was performed using the Dorchester EMS Data Mart, populated from ImageTrend eMeds medication adminstration records for the first 11 month period in 2025.
...
Based on historical usage (7 administration/year), this stocking level provides a substantial buffer while minimizing waste and storage complexity.

---

## Conclusion
Historical medication utilization data supports transitioning primary rate-control stocking away from refrigerated diltiazem when current powdered form diltiazem expires and towards metoprolol. This approach:
- Aligns with state-level protocol guidance
- Reduces cold-chain and expiration management
- Uses objective utilization data to inform purchasing decisions
- Maintains adequate clinical capability for infrequent but high-acuity cases

This case study demonstrates the practical application of EMS data analytics to support clinical, operational, and fiscal decision-making.
